...
**Statement:** If `lens_fit.cooperative` is `good` or `marginal`, the `lens_context.cooperative` block is present and covers membership boundaries, rules source, monitoring mechanism, graduated sanctions, conflict resolution, Ostrom principles addressed, member amendment process, legal form (required at `good`), and a scale fit note confirming governance rules are calibrated to the scale at which the entry is viable. If `lens_fit.civic` is `good` or `marginal`, the `lens_context.civic` block is present and covers political coalition, council vote estimate, relationship to any existing private operator, governance form, municipal budget line, a benchmark comparison of per-household cost against a named analogous civic service, a staffing model (named role, wage referencing `corpus/program/SCALES.md`, FTE, hours, hiring pool), and a civic externalities list naming the public-goods justification for civic investment.

**Rationale:** A `cooperative: good` label without a governance sketch is romantic shorthand, not a claim. Elinor Ostrom's design principles did not emerge from theory — they emerged from watching commons succeed and fail for centuries. An entry that applies the cooperative label without answering "what happens when a member breaks the tool?" or "how can members change the rules?" is a trap. An unregistered cooperative lacking municipal acknowledgment is a documented commons-failure mode. Similarly, a `civic: good` label without a named political coalition, a relationship to existing private operators, a per-household cost benchmark, a staffing model, and a public-goods externalities list is a wish, not a program. The benchmark comparison and externalities list are schema-enforced so P-3 Civic Steward sign-off can be verified mechanically, not left to author discretion.

**Enforces:** P-2 Commons Theorist (coop-good without governance sketch including amendment process and legal form = unacceptable); P-3 Civic Steward (civic-good without political path, cost benchmark, staffing model, and externalities = unacceptable).

**Concrete test:** An entry fails this principle if: `lens_fit.cooperative` is `good` or `marginal` and `lens_context.cooperative` is absent or missing any of {membership_boundary, graduated_sanctions, conflict_resolution, member_amendment_process, scale_fit_note}; `lens_fit.cooperative` is `good` and `legal_form` is absent; `lens_fit.civic` is `good` or `marginal` and `lens_context.civic` is absent or missing any of {political_coalition, competes_with_private, budget_line, benchmark_comparison, staffing_model, civic_externalities}. Placeholder text ("TBD" or "members will figure it out") in these blocks constitutes absence.
...
- [ ] `lens_context` blocks present for every `good` or `marginal` lens label (civic block includes benchmark_comparison, staffing_model, civic_externalities; cooperative block includes member_amendment_process, legal_form when good, scale_fit_note)
