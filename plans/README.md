...
| plan-b-research-smithing | Research Corpus — Smithing | not-yet-authored | 1 | smithing | plan-a-scaffolding | Smithing research corpus is populated with primary and secondary sources covering functional requirements, historical anchor cultures, and material/thermal parameters sufficient to author catalog entries. |
| plan-c-catalog-smithing | Catalog — Smithing | not-yet-authored | 2 | smithing | plan-a-scaffolding, plan-b-research-smithing | All 15 smithing catalog entries exist, pass the panel review gate, have `status: validated`, and carry fully cited `sim_params` blocks ready for Tier A simulation ingestion. |
| plan-d-sim | Simulation and Evaluation Matrix | not-yet-authored | 3 | null | plan-a-scaffolding | Tier A comparator runs all 135 evaluation cells (15 entries × 9 contexts); `results:` blocks are populated in each catalog entry; Tier B and C directories are scaffolded with intent READMEs. |
